...
for (ind in skater_ids2$player_id){
  ...
  for (indx in statlist){
    ...
    trainindex_od = data.frame(createDataPartition(skater_info[indx], p = 0.75, list = F, times = 1))$Resample1 # should train/test selection be random or linear with time?
    train_od = data.frame(skater_info[ ,c(8:20)][trainindex_od, ])
    test_od = data.frame(skater_info[ ,c(8:20)][-trainindex_od,])
...
df_total_pred_pts <- 6*df_total$g + 4*df_total$a + 2*df_total$ppp + (.9)*df_total$sog + 1*df_total$blk
